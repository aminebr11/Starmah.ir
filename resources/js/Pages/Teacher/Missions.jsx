...
export default function Missions() {
    ...
    const blank = { title: '', subject: '', lesson_no: '', difficulty: '', classroom_id: '', question_count: 5, xp_reward: 20, badge_name: '', badge_icon: '🎖️', is_active: true };
                ...
                <form onSubmit={submit}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12 }}>
                        <Field label="عنوان مأموریت" err={form.errors.title}>
                            <input className="input" value={form.data.title} onChange={(e) => form.setData('title', e.target.value)} placeholder="مثلاً: چالشِ ضرب امروز" />
                        </Field>
                    ...
                    {missions.map((m) => (
                        <div key={m.id} style={{ border: '1px solid var(--line)', borderRadius: 14, padding: 13, opacity: m.is_active ? 1 : .55 }}>
                            ...
                            <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 6 }}>
                                {[m.subject || 'همه‌ی درس‌ها', m.lesson_no ? `درس ${m.lesson_no}` : null, m.difficulty ? DIFF[m.difficulty] : null].filter(Boolean).join(' · ')}
                            </div>
